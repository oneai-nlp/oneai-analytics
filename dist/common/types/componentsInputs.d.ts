...
import { Cluster, Item, MetaCluster, MetaData, Phrase, Properties, Trend } from './modals';
import { CountersConfigurations, CounterType, MetadataKeyValue } from './customizeBarTypes';
...
export interface OneAiAnalyticsProps {
    ...
    currentPage?: number;
    nodeClicked?: (node: Omit<OneAIDataNode & {
        id: string;
    }, 'data'>) => void;
...
export type OneAIAnalyticsStaticDataWrapperProps = Omit<OneAiAnalyticsProps & {
    exampleNodes: ExampleNode[];
    collection?: string;
}, 'dataNodes' | 'totalPagesAmount' | 'currentPage'>;
export type OneAIAnalyticsApiWrapperProps = Omit<OneAiAnalyticsProps & {
    domain?: string;
    apiKey?: string;
    collection?: string;
    refreshToken?: string;
